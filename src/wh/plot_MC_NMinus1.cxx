...
//  auto signal_comb_700_1 = Process::MakeShared<Baby_full>("Combined Signal (700,1)", Process::Type::signal, colors("t1tttt"),{signal2016_dir+"slim_*s16v3*.root",signal2017_dir+"slim_*f17v2*.root",signal2018_dir+"slim_*a18v1*.root"},"pass&&mass_stop==700&&mass_lsp==1");
  ...
  vector<shared_ptr<Process> > sample_list_2016 = {tt_2016_all,/*tt2l_2016,tt1l_2016,tt2l_2016_met,tt1l_2016_met,*/single_t_2016,wjets_2016,ttV_2016,SM_WH_2016,diboson_2016};
  vector<shared_ptr<Process> > sample_list_2017 = {tt_2017_all,/*tt2l_2017,tt1l_2017,tt2l_2017_met,tt1l_2017_met,*/single_t_2017,wjets_2017,ttV_2017,SM_WH_2017,diboson_2017};
  vector<shared_ptr<Process> > sample_list_2018 = {tt_2018_all,/*tt2l_2018,tt1l_2018,tt2l_2018_met,tt1l_2018_met,*/single_t_2018,wjets_2018,ttV_2018,SM_WH_2018,diboson_2018};
  // Column combinations all years
  vector<shared_ptr<Process> > sample_list_comb = {tt_comb_all, /*tt2l_Comb, tt1l_Comb, tt2l_Comb_met, tt1l_Comb_met,*/ single_t_Comb, wjets_Comb, ttV_Comb, SM_WH_Comb, diboson_Comb};
  ...
  //Preselection
  NamedFunc basic_cut_SR = "pfmet>125&&mct>200&&mbb>90&&mbb<150&&mt_met_lep>150&&ngoodbtags==2&&nvetoleps==1&&pass&&PassTrackVeto&&PassTauVeto&&ngoodjets==2"&&WHLeptons==1; 
  NamedFunc basic_cut_SR_nomet = "mct>200&&mbb>90&&mbb<150&&mt_met_lep>150&&ngoodbtags==2&&nvetoleps==1&&pass&&PassTrackVeto&&PassTauVeto&&ngoodjets==2"&&WHLeptons==1; 
  NamedFunc basic_cut_SR_nomct = "pfmet>125&&mbb>90&&mbb<150&&mt_met_lep>150&&ngoodbtags==2&&nvetoleps==1&&pass&&PassTrackVeto&&PassTauVeto&&ngoodjets==2"&&WHLeptons==1; 
  NamedFunc basic_cut_SR_nombb = "pfmet>125&&mct>200&&mt_met_lep>150&&ngoodbtags==2&&nvetoleps==1&&pass&&PassTrackVeto&&PassTauVeto&&ngoodjets==2"&&WHLeptons==1; 
  NamedFunc basic_cut_SR_nomt  = "pfmet>125&&mct>200&&mbb>90&&mbb<150&&ngoodbtags==2&&nvetoleps==1&&pass&&PassTrackVeto&&PassTauVeto&&ngoodjets==2"&&WHLeptons==1; 

  vector<NamedFunc> nMinus1Sels = {basic_cut_SR_nomet  , basic_cut_SR_nomct, basic_cut_SR_nombb, basic_cut_SR_nomt};
  vector<string>    xAxes       = {"pfmet"             , "mct"             , "mbb"             , "mt_met_lep"     };
  vector<string>    xLabels     = {"E_{T}^{miss} [GeV]", "mCT [GeV]"       , "mbb [GeV]"       , "mT [GeV]"       };

  // Weights
//  NamedFunc weight_2016  = "w_noBtagSF"*yearWeight;
//  NamedFunc weight_other = "weight"*yearWeight;
  NamedFunc weight = "weight"*yearWeight;
  // Met bins
  ...
  for (uint iyear = 0; iyear < years.size(); iyear++) {
    for (uint isel = 0; isel < nMinus1Sels.size(); isel++) {
      pms[iyear]->Push<Hist1D>(Axis(10, 0, 500., xAxes[isel], xLabels[isel]), nMinus1Sels[isel] && metbins[0], samples_Run2[iyear], all_plot_types).Weight(weight).Tag(years[iyear]);
//      if(iyear == 0) {
// 	pms[iyear]->Push<Hist1D>(Axis(40, 0, 1000., xAxes[isel], xLabels[isel]), nMinus1Sels[isel] && metbins[0], samples_Run2[iyear], all_plot_types).Weight(weight_2016).Tag(years[iyear]);
//      } else {
// 	pms[iyear]->Push<Hist1D>(Axis(40, 0, 1000., xAxes[isel], xLabels[isel]), nMinus1Sels[isel] && metbins[0], samples_Run2[iyear], all_plot_types).Weight(weight_other).Tag(years[iyear]);
//      }
    }
  }

  for(uint iyear = 0; iyear < years.size(); iyear++) {
    pms[iyear]->Push<Hist1D>(Axis(10, 0, 500., "pfmet", "E_{T}^{miss} [GeV]"), basic_cut_SR && metbins[0], samples_Run2[iyear], all_plot_types).Weight(weight).Tag(years[iyear]);
//    if(iyear == 0) {
//      pms[iyear]->Push<Hist1D>(Axis(5, 0, 1000., "pfmet", "E_{T}^{miss} [GeV]"), basic_cut_SR && metbins[0], samples_Run2[iyear], all_plot_types).Weight(weight_2016).Tag(years[iyear]);
//    } else {
//      pms[iyear]->Push<Hist1D>(Axis(5, 0, 1000., "pfmet", "E_{T}^{miss} [GeV]"), basic_cut_SR && metbins[0], samples_Run2[iyear], all_plot_types).Weight(weight_other).Tag(years[iyear]);
//    }
  }

  string plotPath = "nMinus1/";
