...
int main(){
 ...
 // string mc_dir_old_tt1l = "/home/users/rheller/wh_babies/babies_2019_02_11/";
  string mc_dir_old = "/home/users/rheller/wh_babies/babies_v30_9_2019_04_03/";
  string mc_dir_new = "/home/users/rheller/wh_babies/babies_v31_2_2019_07_24/";
  string mc_dir_met = "/home/users/rheller/wh_babies/babies_v31_1_2019_07_24/";
  ...
  auto tt1l_old = Process::MakeShared<Baby_full>("t#bar{t} (1l), old", Process::Type::data, colors("data"),
    {mc_dir_new+"*TTJets_1lep_top_s16v3*.root",mc_dir_new+"*TTJets_1lep_tbar_s16v3*"});
  auto tt1l_new = Process::MakeShared<Baby_full>("t#bar{t} (1l), new inclusive", Process::Type::background, colors("tt_1l"),
    {mc_dir_new+"*TTJets_1lep_top_s16v3*.root",mc_dir_new+"*TTJets_1lep_tbar_s16v3*"});
  tt1l_new->SetFillColor(kWhite);
  tt1l_new->SetLineColor(kAzure-2);
  tt1l_new->SetLineWidth(3);
  auto tt1l_met = Process::MakeShared<Baby_full>("t#bar{t} (1l), gen met > 150", Process::Type::data, colors("data"),
    {mc_dir_met+"*TTJets_1lep_*met150*.root"});


 auto tt2l_old = Process::MakeShared<Baby_full>("t#bar{t} (2l), old", Process::Type::data, colors("data"),
{mc_dir_old+"*TTJets_2lep_s16v3*.root"});

  auto tt2l_new = Process::MakeShared<Baby_full>("t#bar{t} (2l), new inclusive", Process::Type::background, colors("tt_2l"),
    {mc_dir_new+"*TTJets_2lep_s16v3*.root"});
  tt2l_new->SetFillColor(kWhite);
  tt2l_new->SetLineColor(kAzure-2);
  tt2l_new->SetLineWidth(3);
  // auto tt2l_new = Process::MakeShared<Baby_full>("t#bar{t} (2l), v30_9", Process::Type::data, colors("data"),
  //   {mc_dir_new+"*TTJets_2lep_s16v3*.root"});
 
   auto tt2l_met = Process::MakeShared<Baby_full>("t#bar{t} (2l), gen met > 150", Process::Type::data, colors("data"),
    {mc_dir_met+"*TTJets_2lep_met150*.root"});
...
//  vector<shared_ptr<Process> > sample_list = {data, /*tchiwh_250_1, tchiwh_225_75,*/ tt1l, tt2l, wjets_low_nu, wjets_high_nu, single_t, ttv, diboson};
  ...
  vector<vector<shared_ptr<Process> >> sample_list_list ={/*{tt1l_old,tt1l_new},{tt1l_met,tt1l_new},*/{tt2l_old,tt2l_new},{tt2l_met,tt2l_new}}; 
  vector<string> tag_list = {/*"tt1l_inclu","tt1l_met",*/"tt2l_inclu","tt2l_met"};
  ...
  vector<string> weights = {"1","w_noBtagSF*w_BtagSF_medmed"};
  ...
  for(uint i=0;i<sample_list_list.size();i++){
    ...
    pm.Push<Hist1D>(Axis(20, 300, 700., "pfmet", "E_{T}^{miss} [GeV]"),
        sig && "genmet>150", sample_list_list[i], all_plot_types).Weight(weights[j]).Tag(tag_list[i]);  
