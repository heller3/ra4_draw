...
void MakeLimitPlot(vector<double> vmx,
                   ...
                   const TH2D &hsigexp){
  SetupColors();
  vector<string> tokens = Tokenize(filename_,"/");
  string variation = tokens[0];
  string xparticle, yparticle;
  GetParticleNames(xparticle, yparticle);
...
//  l.AddEntry(&cobs, "Observed", "l");
  ...
  string filebase = model_+"_"+variation+"_limit_scan";
  if(num_smooth_>0){
    filebase += "_smooth";
...
TGraph DrawContours(TGraph2D &g2, int color, int style, double width,
  ...
  if(n_smooth>0){
    ...
    cout<<"xmin "<<histo2d->GetXaxis()->GetXmin()<<"ymin "<<histo2d->GetYaxis()->GetXmin()<<endl;
    cout<<"xmax "<<histo2d->GetXaxis()->GetXmax()<<"ymax "<<histo2d->GetYaxis()->GetXmax()<<endl;
    TH2D htemp("", "",
  ...
  thresh=140;
	if(x-y>thresh){
      ...
      //cout<<"Discarding point "<<x<<" "<<y<<endl;
...
void FixGraph(TGraph &graph){
  ...
  cout<<"endlsp, endlgu is "<<endlsp<<" "<<endglu<<endl;
